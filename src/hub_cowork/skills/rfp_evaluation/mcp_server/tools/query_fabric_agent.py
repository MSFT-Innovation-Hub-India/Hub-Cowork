...
def _get_credential(tenant_id: str, auth_mode: str):
    ...
    # tenant as the Fabric resource. The shared credential is bound to
    ...
    try:
        ...
        if shared_tenant and tenant_id and shared_tenant.lower() == tenant_id.lower():
            ...
            if shared is not None:
                logger.info("[FabricAgent] Reusing shared agent credential (tenant match, silent refresh)")
    ...
    if auth_mode == "cli":
        ...
        logger.info("[FabricAgent] Using %s credential for tenant %s",
